      ...
      <div className="grid gap-3.5">
        {ROWS.map(([from, to], i) => (
          <Reveal
            key={from}
            delay={i * 0.06}
            className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] items-center gap-4 sm:gap-[18px] rounded-[18px] border border-[#e3e7f1] bg-[#f6f7fb] px-5 py-5 sm:px-[22px] text-center sm:text-left"
          >
            <div className="font-outfit font-semibold text-[15px] text-[#0e1a3c]">{from}</div>
            <div className="text-primary text-[20px] rotate-90 sm:rotate-0 justify-self-center">→</div>
            <div className="font-jakarta text-[14.5px] text-[#3f4a63]">{to}</div>
          </Reveal>
        ))}
      </div>
