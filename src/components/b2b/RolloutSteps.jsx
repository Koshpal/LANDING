...
export default function RolloutSteps() {
  return (
    <Section tone="white">
      ...
      <Reveal className="grid md:grid-cols-3 border border-[#e3e7f1] rounded-[18px] overflow-hidden bg-[#f6f7fb] b2b-shadow">
        {STEPS.map(([num, h, p, flow], i) => (
          <div
            key={num}
            className={`p-[26px] sm:p-[30px] ${i < 2 ? "border-b md:border-b-0 md:border-r border-[#e3e7f1]" : ""}`}
          >
            <div className="font-outfit font-extrabold text-[14px] tracking-[0.1em] text-secondary-darkest">{num}</div>
            <h3 className="font-outfit font-semibold text-[20px] text-[#0e1a3c] my-2.5">{h}</h3>
