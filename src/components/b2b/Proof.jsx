...
export default function Proof() {
  return (
    <Section tone="white" id="proof">
      ...
      <div className="mb-12">
        <p className="font-outfit font-semibold text-[12px] tracking-[0.14em] uppercase text-[#6b7597] mb-5">
        ...
        <div className="flex flex-wrap items-center gap-x-10 gap-y-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-9 min-w-[132px] grid place-items-center rounded-lg border border-dashed border-[#dfe3f2] bg-[#f7f8fd] font-outfit font-semibold text-[11px] tracking-[0.06em] text-[#8a95b4]"
            >
              CLIENT LOGO
            </div>
          ))}
      ...
      <div className="grid md:grid-cols-3 gap-[18px]">
        {[
          ...
          <Reveal
            key={slot}
            delay={i * 0.06}
            className="rounded-[20px] border border-dashed border-[#dfe3f2] bg-[#f7f8fd] p-6 min-h-[180px] flex flex-col justify-between"
          >
